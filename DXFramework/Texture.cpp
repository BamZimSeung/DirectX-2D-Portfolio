...
map<wstring, PDIRECT3DTEXTURE9> Texture::textureMap;
...
void Texture::CreateVertexBuffer()
{
	...
	for (size_t i = 0; i < textures.size(); i++) {
		...
		D3DXVECTOR2 tempSize = size / 2;

		vertex[i * 6 + 0] = Vertex(-tempSize.x, -tempSize.y, start.x, start.y);
		vertex[i * 6 + 1] = Vertex(tempSize.x, -tempSize.y, end.x, start.y);
		vertex[i * 6 + 2] = Vertex(-tempSize.x, tempSize.y, start.x, end.y);
		vertex[i * 6 + 3] = Vertex(-tempSize.x, tempSize.y, start.x, end.y);
		vertex[i * 6 + 4] = Vertex(tempSize.x, -tempSize.y, end.x, start.y);
		vertex[i * 6 + 5] = Vertex(tempSize.x, tempSize.y, end.x, end.y);
	}
